...
export const CardSiteSecondary = ({ imagem, titulo, icone_seta, texto, icone_bola, item1, item2, item3, item4 }) => {
    const isLink = titulo.startsWith('http');
    ...
    return (
        <section className='content-secondary-card-site'>
            ...
            {/* Se for link, envolvemos em uma tag <a> */}
            <div className='box-title-secondary-card-site'>
                {isLink ? (
                    <a href={titulo}
                        target="_blank" rel="noopener noreferrer"
                        style={{ display: 'flex', alignItems: 'center', textDecoration: 'none', color: 'inherit', gap: '10px' }}>
                        <Image
                            className='image-title-secondary-card-site'
                            src={icone_seta} alt="" />
                            <h2>{titulo}</h2>
                    </a>
                ) : (
